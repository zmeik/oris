...
## Быстрый старт — два варианта demo

### Вариант 1: Production Arena clone + mock backend (РЕКОМЕНДУЕТСЯ — full UI)

**Побайтовый клон** реального Darwin-Lab Arena из проекта X-RayAnalizer (14 JS-модулей, ~960 КБ), работающий против Flask mock-бэкенда с синтетическими данными:
...
3 синтетических ORIS-примера загружаются автоматически. Вы получаете **полный клинический интерфейс** — 18-status cycle picker, surface markup (m/d/o/v/l), anatomy/TMJ/airway панели, algorithm comparison, time-machine GT history, D3 evolutionary tree.

См. [`production-arena/README.md`](production-arena/README.md) для деталей.

### Вариант 2: Schema test mode (лёгкий, без установки — только для exploration схемы)

Упрощённый single-page browser-инструмент для изучения схемы без backend:
...
Это **упрощённая** тестовая поверхность (без algorithm comparison, без anatomy SVG, без D3 tree) — достаточно для клика по клеткам, выбора статусов и просмотра live JSON. Для полного клинического интерфейса используйте Вариант 1.

⚠️ **Обе demo используют только синтетические данные.** Не вводите реальную информацию о пациентах. См. [PRIVACY.md](PRIVACY.md) для compliance (Федеральный закон РФ № 152-ФЗ, GDPR, HIPAA-equivalent practices).
...
└── web-demo/ ...................... тестовый режим в браузере (EN + RU)
```
