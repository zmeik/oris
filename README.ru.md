...
## Быстрый старт — Web demo (без установки)

Самый быстрый способ увидеть, как выглядит ORIS — встроенный в репозиторий **тестовый режим в браузере**:
...
Demo работает полностью в твоём браузере (без сервера, без отправки данных). Ты можешь:

- Кликать по 32 dental cells, выбирать статус из cycle, отмечать поверхности (m/d/o/v/l)
- Заполнять anatomy panel (нижнечелюстной канал, пазухи, TMJ)
- Видеть live JSON output, формирующийся по мере кликов
- Переключать UI язык между **English** и **Русский**
- Экспортировать получившийся ORIS-документ как JSON-файл

⚠️ **Demo использует только синтетические данные.** Не вводите реальную информацию о пациентах. См. [PRIVACY.md](PRIVACY.md) для compliance-деталей (Федеральный закон РФ № 152-ФЗ, GDPR, HIPAA-equivalent practices).
